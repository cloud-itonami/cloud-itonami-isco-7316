(ns signcraft.governor
  "SignCraftGovernor — the independent safety/scope layer gating every
  workshop scheduling/logistics proposal an advisor may make for a
  sign, decorative-painting, engraving and etching shop crew. The
  governor never dispatches hardware itself, never performs
  signwriting, decorative-painting, engraving or etching work on the
  shop floor, and never finalizes a painting/engraving-execution
  ...
  SCHEDULING/LOGISTICS ONLY — it never performs painting, engraving or
  etching work itself). Modeled on cloud-itonami-isco-7321's
  prepresstech.governor for the precision-craft/chemical-hazard-
  workshop shape.
  ...
  HARD invariants (:hard? true, ALWAYS :hold, never overridable):
    ...
    3. no-actuation           — proposal :effect must be :propose (the
                                ...
                                never performs painting, engraving or
                                etching work itself; it only gates
    ...
    5. scope-excluded action  — any proposal to directly finalize a
                                painting/engraving-execution decision
                                (e.g. deciding to proceed with a
                                specific signwriting, decorative-
                                painting, engraving or etching
                                procedure), or a
                                workshop-chemical-safety-clearance
                                decision, or to override a shop safety
                                ...
                                \"declare the workshop chemical-safety
                                cleared\" / \"override the shop safety
                                officer's judgment\", never as bare
                                nouns like \"paint\", \"solvent\",
                                ...
                                crew operation for signwriting/
                                decorative-painting task …\" or
  ...
  regardless of confidence):
    6. :op :flag-safety-concern (a VOC/paint-solvent/etching-chemical-
                                exposure or equipment-condition concern
...
(def ^:private scope-excluded-ops
  #{:finalize-painting-execution-decision :finalize-engraving-execution-decision
    :authorize-painting-operation :authorize-engraving-operation
    :proceed-with-painting-execution-procedure :proceed-with-engraving-execution-procedure
...
;; ("paint", "solvent", "engraving", "etching", "chemical", "safety",
...
;; those bare nouns, e.g. "signwriting/decorative-painting task" /
;; "shop safety officer review"). See
;; `governor-test/default-mock-advisor-proposals-never-self-trip-on-scope-exclusion`.
(def ^:private scope-excluded-phrases
  ["proceed with the painting operation" "proceed with the painting execution procedure"
   "proceed with the engraving operation" "proceed with the engraving execution procedure"
   "proceed with the etching operation" "proceed with the etching execution procedure"
   "finalize the painting operation" "finalize the painting execution decision"
   "finalize the engraving operation" "finalize the engraving execution decision"
   "finalize the etching operation" "finalize the etching execution decision"
   "authorize the painting operation" "authorize the engraving operation"
   "authorize the etching operation"
   "declare the workshop chemical-safety cleared" "declare the workshop chemical safety cleared"
   "declare workshop chemical-safety cleared" "declare the workshop as chemical-safety cleared"
  ...
  (let [{:keys [op rationale]} proposal]
    (cond-> []
      ...
      (conj {:rule :scope-excluded-action
             :detail "彩色・彫刻・エッチングの実行判断の確定／工房の薬品安全クリアランス判断の確定・shop safety officer の判断の上書きは、この actor の権限外 — 常に永続ブロック"}))))

(defn check
  "Assess a proposal against `request`/`context`/`proposal` and a
  `store` implementing `signcraft.store/Store`. Pure — never mutates
